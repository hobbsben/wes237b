...
/* simple square window FIR */
//Modified for WES237B
{
	int i_in=0, i_out=0;
	int in_rate = (int)s->rate_out; //fast
	int out_rate = s->rate_out2; //slow
	int ratio = in_rate/out_rate;
/*

while i_in is smaller than s->result:
	increment s->now_lpr by s->result[i_in]

	increment i_in by 1

	increment s->prev_lpr_index by out_rate

	if s->prev_lpr_index is smaller than in_rate go to start of the while loop (continue)

	calculate the output and place it in s->result[i_out] - it should be cast as int16_t

	decrease s->prev_lpr_index by in_rate; 
	set s->now_lpr to zero
	increment i_out by 1

*/
//PB

while(i_in < s->result_len)
{
s->now_lpr=s->now_lpr + s->result[i_in];
i_in++;
s->prev_lpr_index+= out_rate;
if(s->prev_lpr_index < in_rate)
{continue;}
s->result[i_out]=(int16_t)(s->now_lpr/(ratio));
s->prev_lpr_index=s->prev_lpr_index - in_rate;
s->now_lpr=0;
i_out++;
}
