...
struct Profile
{
	...
	Profile():time_init(0), time_cpu(0), time_gpu(0), time_rmse(0), time_total(0){}
};
...
// ----------------------------------------------
void fir_cpu(const float *coeffs, const float *input, float *output, int length, int filterLength)
// ----------------------------------------------
{
...
// ----------------------------------------------
{
	...
	/*coeffs      = */(float*)cudaMallocManaged(&coeffs,FILTER_LEN * sizeof(float));
	/*paddedInput = */(float*)cudaMallocManaged(&paddedInput,PADDED_SIZE * sizeof(float));
	/*output2     = */(float*)cudaMallocManaged(&output2,OUTPUT_SIZE * sizeof(float));
